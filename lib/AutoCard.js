      ...
      .then(data => {
        ...
        data.forEach(typeData => {
          ...
          typeData.cards.forEach(card => {
            let cardElm;
            if (showImages) {
              cardElm = document.createElement('auto-card-image')
            } else {
              cardElm = document.createElement('auto-card')
            }
            category.appendChild(cardElm)
            cardElm.setAttribute('name', card.card.name)
            cardElm.setAttribute('quantity', card.quantity)
            cardElm.setAttribute('imgSource', self.getAttribute('imgSource'))
            cardElm.setAttribute('linkSource', self.getAttribute('linkSource'))
            if (preview) {
