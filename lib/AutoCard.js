    ...
    DeckList.DeckListFromUrl(listSrc, options, cardApi)
      .then(data => {
        ...
        if (!showImages) {
          ...
          const imageList = document.createElement('auto-card-list-images')
          data.forEach(typeData => {
            typeData.cards.forEach(card => {
          ...
          self.container.appendChild(imageList)
        }
      })
  }
}
...
class CardListImages extends HTMLElement {}
...
customElements.define('auto-card-list-images', CardListImages)
