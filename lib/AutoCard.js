    ...
    return this.getCardInfo().then(cardInfo => {
      ...
      switch (config.linkSource) {
        ...
        case Constants.linkSource.urzaCo:
          return 'https://urza.co/cards/search?q=' + name
        default:
...
class CardList extends _TextListener {
  ...
  renderBody(deckData) {
    ...
    if (hasChanges) {
      ...
      this.renderDecklist(cuts, diff, 'Cuts');
      this.renderDecklist(adds, diff, 'Adds');
    ...
    if (deckList && !isCollapsed) {
      this.renderDecklist(deckList, this, hasChanges && 'Deck List');
    }
  }
  renderDecklist(deckList, parent, title) {
