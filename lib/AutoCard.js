...
class CardList extends _TextListener {
  ...
  renderHead() {
    ...
    return self.deckDataPromise.then(deckData => {
      const { meta } = deckData
      const deckName = self.getAttribute('name') || meta.name
      if (deckName){
        ...
        self.appendChild(listHead)
        listHeadText.innerHTML = deckName

        if (self.canCollapse) {
          listHead.appendChild(self.listToggle)
        }
      }
    })
  }
  renderBody() {
    ...
    self.appendChild(listBody)
    listBody.innerHTML = 'loading deck list...'
