...
class CardList extends _TextListener {
  onInitTextListener() {
    ...
    if (listSrc) {
      this.deckDataPromise = DeckList.DeckListFromUrl(listSrc, this.deckOptions, cardApi)
      this.createElements()
    }
  }
  handleInnerText(textContent) {
    if (!this.deckDataPromise) {
      this.deckDataPromise = DeckList.DeckListFromText(textContent, this.deckOptions, cardApi)
      this.createElements()
    }
  }
  createElements() {
