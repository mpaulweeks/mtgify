...
const PROMO_SET_TYPES = [
  "masterpiece",
]
function handleAllSetsStream(cardHandler) {
  return set => {
    if (!PROMO_SET_TYPES.includes(set.type)) {
      set.cards.forEach(card => cardHandler(set, card))
    }
    return set
  }
}
...
function generateMultiverseJson(){
  const mids = {}
  const midsLower = {}
  const handler = handleAllSetsStream((set, card) => {
...
function generateCardInfoLowerJson(){
  ...
  return new Promise((resolve, reject) => {
    stream.on('end', () => {
      ...
      fs.writeFile('./json/CardInfo.lower.json', JSON.stringify(lookup), 'utf8', () => {
        console.log('generating CardInfo.compressed.json')
        const compressed = Compressor.compressArray(Object.values(lookup))
        console.log('writing CardInfo.compressed.json')
        fs.writeFile('./json/CardInfo.compressed.json', JSON.stringify(compressed), 'utf8', resolve)
      })
    })
  })
}
...
function generateAllCustomJson(){
  return generateCardInfoLowerJson()
