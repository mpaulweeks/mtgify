...
function updateVersion(){
  return fetchVersion()
    .then(remoteVersion => {
      ...
      if (needsUpdate) {
        console.log('new version', remoteVersion.toJSON())
        saveVersion(remoteVersion)
      } else {
        console.log('same version', remoteVersion.toJSON())
      }
      return needsUpdate
    })
}
...
function generateAllCardsLowerJson(){
  ...
  return new Promise((resolve, reject) => {
    stream.on('end', () => {
      ...
      fs.writeFile('./json/AllCards.lower.json', JSON.stringify(lower), 'utf8', resolve)
    })
  })
}
