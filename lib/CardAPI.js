...
class CardAPI {
  constructor() {
    ...
    window.fetch(`${Config().apiUrl}/json/CardInfo.compressed.json`, fetchSettings)
      .then(res => res.json())
      .then(cardData => self.resolveLookup(cardData))
  }
  getVersion(customConfig) {
    const config = Config(customConfig)
    return window.fetch(`${config.apiUrl}/json/version.json`, fetchSettings)
      .then(resp => resp.json())
  }
  ...
  attachCardMetaData(cardName, card, config) {
    ...
    switch (config.imgSource) {
      case Constants.imgSource.magicCardsInfo:
        imgUrl = `https://magiccards.info/scans/en/${card.mciSet}/${card.mciNum}.jpg`
        break
      case Constants.imgSource.scryfall:
        imgUrl = `https://img.scryfall.com/cards/normal/en/${card.setCode}/${card.cardNum}.jpg`
        break
      case Constants.imgSource.urzaCo:
        imgUrl = `https://s3.urza.co/cards/${card.setCode}/front/200dpi/${pad(card.cardNum, 3)}.jpg`
        break
    }
    if (!imgUrl){
