...
class CardAPI {
  constructor(customConfig) {
    ...
    window.fetch(`${config.apiUrl}/json/version.json`, { cache: "no-store" })
      .then(resp => resp.json())
      .then(version => (
        // one-time cache bust because CORS was broken before 20190714
        window.fetch(`${config.apiUrl}/json/CardInfo.compressed.json?v=20190714-${version.version}`)
          .then(res => res.json())
  ...
  attachCardMetaData(cardName, card, config) {
    ...
    switch (config.imgSource) {
      ...
      case Constants.imgSource.scryfall:
        ...
        imgUrl = `https://api.scryfall.com/cards/${card.setCode}/${card.cardNum}?format=image`
        break
