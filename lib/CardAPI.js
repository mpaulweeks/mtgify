...
class CardAPI {
  constructor(customConfig) {
    ...
    window.fetch(`${config.apiUrl}/json/version.json`, { cache: "no-store" })
      .then(resp => resp.json())
      .then(version => {
        const v = version.date.split('-').join('');
        return window.fetch(`${config.apiUrl}/json/CardInfo.compressed.json?v=${v}`)
          .then(res => res.json())
  ...
  attachCardMetaData(cardName, card, config) {
    ...
    switch (config.imgSource) {
      ...
      case Constants.imgSource.scryfall:
        ...
        imgUrl = `https://api.scryfall.com/cards/${card.setCode}/${card.cardNum}/${lang}?format=image`
        break
  ...
  renderAutoCard(card, config) {
    ...
    node.setAttribute('lang', config.lang)
    return node
  }
