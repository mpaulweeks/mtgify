...
describe('DeckList module', () => {
  const T = {}
  beforeAll(() => {
    T.cardLookup = fetchCardLookup()
    T.text = fs.readFileSync('./test/sample_deck.txt', "utf8")
  })
  test('returns all cards', () => {
    const sut = new DeckList.DeckList(T.text, {}, T.cardLookup)
  ...
  test('options.reverse = false', () => {
    const sut1 = new DeckList.DeckList(T.text, {}, T.cardLookup)
  ...
  test('options.reverse = true', () => {
    const sut2 = new DeckList.DeckList(T.text, {reverse: true}, T.cardLookup)
