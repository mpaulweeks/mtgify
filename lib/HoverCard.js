...
function show(elm) {
  elm.getMultiverse()
    .then(multi => {
      hp.setAttribute('data-mid', multi.mid)

      hp.style.top = getTop(elm)+"px"
      hp.style.left = determineLeft(elm)+"px"

      hp.innerHTML = "<img src=\""+multi.imgUrl+"\" onerror=\"this.onerror=null;this.onmouseout=null;this.onmouseover=null;hideImgPopup();\" width=\""+CARD_WIDTH+"\" height=\""+CARD_HEIGHT+"\"/>"
...
function hide(elm) {
  elm.getMultiverse()
    .then(multi => {
      if (hp.getAttribute('data-mid') === String(multi.mid)) {
